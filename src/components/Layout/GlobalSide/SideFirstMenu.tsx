...
class SideFirstMenu extends React.Component<SideFirstMenuProps, SideFirstMenuState> {
    protected getAntdMenu() {
        ...
        // 菜单项
        const menuNodes = getAntdMenuItems({
            showRootMenus: layoutMenuData.showRootMenus!,
            menuIconScriptUrl,
            menuItemRender,
            menuItemClassName,
            menuItemStyle
    ...
    protected getCustomMenuNode(): React.ReactNode[] {
        ...
        if (showRootMenus && showRootMenus.length >= 0) {
            showRootMenus.forEach((menu) => {
                ...
                let active = false;
                if (
                    selectedProps.defaultSelectedKeys &&
                    selectedProps.defaultSelectedKeys.findIndex((key) => key === menu.menuKey) !== -1
                ) {
                    active = true;
                }
                ...
                const node = (
                    <div
                        ...
                        className={classNames(
                            ...
                            { [styles.customMenuItemActive]: active },
                            { [styles.customMenuItemCompact]: true },
                            menuItemClassName
                        )}
                        style={menuItemStyle}
                        onClick={(event) => {
