    ...
    protected getMenuNode(menus?: RuntimeMenuItem[]): React.ReactNode[] {
        ...
        menuGroupMap.forEach((menuGroup, groupName) => {
                ...
                if (menu.children && menu.children.length > 0) {
                    ...
                    menuNodeArray.push(
                        <Menu.SubMenu
                            key={menu.menuKey}
                            title={
                                menuFolderRender instanceof Function
                                    ? menuFolderRender(menu, icon)
                                    : this.defaultMenuFolderRender(menu, icon)
                            }
            ...
            if (groupName) {
                // 菜单需要分组
                const title =
                    menuItemGroupRender instanceof Function
                        ? menuItemGroupRender(menuGroup, groupName)
                        : this.defaultMenuItemGroupRender(menuGroup, groupName);
                nodes.push(
