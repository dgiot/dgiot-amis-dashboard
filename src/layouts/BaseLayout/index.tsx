...
interface BaseLayoutProps extends LayoutPageComponentProps {
    ...
    globalHeaderLeftRender?: (
        props: Omit<GlobalHeaderProps, 'leftRender'>,
        className: string,
        elementMap: Map<String, React.ReactNode>
    ) => React.ReactNode;
    /** 自定义中间动态宽度区域渲染逻辑 */
    globalHeaderCentreRender?: (
        props: Omit<GlobalHeaderProps, 'centerRender'>,
        className: string,
        elementMap: Map<String, React.ReactNode>
    ) => React.ReactNode;
    /** 自定义右侧区域渲染逻辑 */
    globalHeaderRightRender?: (
        props: Omit<GlobalHeaderProps, 'rightRender'>,
        className: string,
        elementMap: Map<String, React.ReactNode>
    ) => React.ReactNode;
    /** 自定义渲染逻辑 */
    ...
    /** 自定义中间动态宽度区域渲染逻辑 */
    sideMenuCentreRender?: (
        props: Omit<SideMenuProps, 'centreRender'>,
        className: string,
        elementMap: Map<String, React.ReactNode>
    ) => React.ReactNode;
    /** 自定义底部区域渲染逻辑 */
    sideMenuBottomRender?: (
        props: Omit<SideMenuProps, 'bottomRender'>,
        className: string,
        elementMap: Map<String, React.ReactNode>
    ) => React.ReactNode;
    /** 自定义渲染逻辑 */
...
class BaseLayout<P extends BaseLayoutProps, S extends BaseLayoutState> extends React.Component<P, S> {
    ...
    protected getEditCodeButton() {
        ...
        // console.log((location as any).search===undefined);
        // console.log((location as any).search.includes("debug=true"));
        ...
        return (
            <div className={styles.editCode}>
                {(location as any).search === undefined ? (
                    ''
    ...
    protected getSideMenu(defaultSideMenuBottomRender?: DefaultSideMenuBottomRender, defaultSideMenuTopRender?: DefaultSideMenuTopRender) {
        ...
        const {
            ...
            layoutMenuData,
            sideMenuOnSearchMenu,
            sideMenuOnSearchValueChange,
            sideMenuOnMenuClick,
            sideMenuOnMenuOpenChange
        } = this.props;
        ...
        // 保存当前二级菜单
        if (!layoutMenuData.currentMenu.isHide)
        ...
        if (!sideMenuBottomRender && defaultSideMenuBottomRender instanceof Function) {
            sideMenuBottomRender = (props, className, elementMap) =>
                defaultSideMenuBottomRender!(props, className, elementMap, currentFirstMenu);
        }
    ...
    protected getPageContent() {
        ...
        const delArr = lodash
            .keys(window.amisPages)
            .filter((amisPageName) => multiTabs.findIndex((tab) => amisPageName === tab.amisPageName) < 0);
    ...
    protected addOrShowTabPage() {
            ...
            if (activePageKey !== multiTabKey) {
                ...
                if (multiTab.pageType === 'react') {
                    ...
                    if (multiTab.amisPageName && window.amisPages && window.amisPages[multiTab.amisPageName]) {
                        ...
                        const globalData: AmisPageGlobalData = {
                            menuItem: multiTab.menuItem,
                            location: multiTab.location,
                            match: multiTab.match
                        };
        ...
        log.info(
            'amisId -> ',
            newMultiTab.mountedDomId,
            'routerName -> ',
            name,
            'pagePath -> ',
            pagePath,
            'pageType -> ',
            newMultiTab.pageType
        );
        if (newMultiTab.pageType === 'amis') window.currentAmisId = newMultiTab.mountedDomId;
        this.setState({ activePageKey: multiTabKey, multiTabs, showEditCodeModal: false }, async () => {
            if (newMultiTab.pageType === 'react') {
                ...
                // 初始化全局数据
                const globalData: AmisPageGlobalData = {
                    menuItem: newMultiTab.menuItem,
                    location: newMultiTab.location,
                    match: newMultiTab.match
                };
