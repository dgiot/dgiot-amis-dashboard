...
} from '@/components/Layout/GlobalSide';
import {
    getCurrentFirstMenu,
...
interface NestSideMenuLayoutProps extends BaseLayoutProps {
    ...
    /** 自定义顶部区域渲染逻辑 */
    globalSideTopRender?: (
        props: Omit<GlobalSideProps, 'topRender'>,
        className: string,
        elementMap: Map<String, React.ReactNode>
    ) => React.ReactNode;
    /** 自定义中间动态宽度区域渲染逻辑 */
    globalSideCentreRender?: (
        props: Omit<GlobalSideProps, 'centreRender'>,
        className: string,
        elementMap: Map<String, React.ReactNode>
    ) => React.ReactNode;
    /** 自定义底部区域渲染逻辑 */
    globalSideBottomRender?: (
        props: Omit<GlobalSideProps, 'bottomRender'>,
        className: string,
        elementMap: Map<String, React.ReactNode>
    ) => React.ReactNode;
    /** 自定义渲染逻辑 */
...
class NestSideMenuLayout extends BaseLayout<NestSideMenuLayoutProps, NestSideMenuLayoutState> {
    ...
    protected getGlobalSide() {
        ...
        const {
            menuIconScriptUrl,
            globalSideLogo,
            globalSideOnLogoClick,
            globalSideLogoClassName,
            globalSideLogoStyle,
            globalSideMenuMode,
            globalSideDefaultSelectedKeys,
            globalSideSelectedKeys,
            globalSideMenuItemRender,
            globalSideOnMenuSelect,
            globalSideOnMenuClick,
            globalSideMenuClassName,
            globalSideMenuStyle,
            globalSideMenuItemClassName,
            globalSideMenuItemStyle,
            globalSideMenuProps,
            globalSideTopClassName,
            globalSideTopStyle,
            globalSideCentreClassName,
            globalSideCentreStyle,
            globalSideBottomClassName,
            globalSideBottomStyle,
            globalSideTopRender,
            globalSideCentreRender,
            globalSideBottomRender,
            globalSideGlobalSideRender,
            globalSideForceHideScrollbar,
            globalSideAutoHideScrollbar,
            globalSideScrollbarClassName
        } = this.props;
            ...
            <div className={classNames(className, topClassName)} style={topStyle}>
                <div className={styles.sideMenuCollapse}>
                    ...
                    <CollapseCursor
                        ...
                        className={styles.sideMenuCollapseCursor}
                    />
        ...
        return (
            <section className={classNames(styles.layout, layoutClassName)} style={layoutStyle}>
                <aside
                    ...
                    style={{
                        ...(globalSideMenuMode === SideFirstMenuMode.CustomMenu ? { width: globalSideMenuWidth } : {}),
                        ...globalSideStyle
                    }}
                >
                    {/* 全局侧边栏 - 一级菜单 */}
                ...
                <section className={classNames(styles.nestLayout, nestLayoutClassName)} style={nestLayoutStyle}>
                    ...
                    {currentFirstMenu && currentFirstMenu.children && currentFirstMenu.children.length > 0 && (
                        <aside
                            className={classNames(styles.layoutSide, sideClassName)}
                            style={{ ...sideStyle, width: this.getMenuCollapsed() ? 48 : sideMenuWidth }}
                        >
                    ...
                    <section className={classNames(styles.twoLevelNestLayout, twoLevelNestLayoutClassName)} style={twoLevelNestLayoutStyle}>
                        ...
                        {!hideGlobalHeader && (
                            <header
                                ...
                                style={{
                                    ...headerStyle,
                                    ...(headerHeight ? { height: headerHeight, lineHeight: `${headerHeight}px` } : {})
                                }}
                            >
